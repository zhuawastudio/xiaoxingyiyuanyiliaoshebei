package com.entity.view;

import org.apache.tools.ant.util.DateUtils;
import com.annotation.ColumnInfo;
import com.entity.KufangEntity;
import com.baomidou.mybatisplus.annotations.TableName;
import org.apache.commons.beanutils.BeanUtils;
import java.lang.reflect.InvocationTargetException;
import org.springframework.format.annotation.DateTimeFormat;
import com.fasterxml.jackson.annotation.JsonFormat;
import java.io.Serializable;
import java.util.Date;
import com.utils.DateUtil;

/**
* 库房
* 后端返回视图实体辅助类
* （通常后端关联的表或者自定义的字段需要返回使用）
*/
@TableName("kufang")
public class KufangView extends KufangEntity implements Serializable {
	private static final long serialVersionUID = 1L;

	//当前表
	/**
	* 库房类型的值
	*/
	@ColumnInfo(comment="库房类型的字典表值",type="varchar(200)")
	private String kufangValue;




	public KufangView() {

	}

	public KufangView(KufangEntity kufangEntity) {
		try {
			BeanUtils.copyProperties(this, kufangEntity);
		} catch (IllegalAccessException | InvocationTargetException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}



	//当前表的
	/**
	* 获取： 库房类型的值
	*/
	public String getKufangValue() {
		return kufangValue;
	}
	/**
	* 设置： 库房类型的值
	*/
	public void setKufangValue(String kufangValue) {
		this.kufangValue = kufangValue;
	}




	@Override
	public String toString() {
		return "KufangView{" +
			", kufangValue=" + kufangValue +
			"} " + super.toString();
	}
}
